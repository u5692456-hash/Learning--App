import { generateJavaScriptTemplate, generateGenericTemplate } from './pdfTemplates';
...
export const generateCourseFromPdf = (file: File): GeneratedCourse => {
  ...
  if (fileName.includes('javascript') || fileName.includes('js') || fileName.includes('react') || fileName.includes('node')) {
    template = generateJavaScriptTemplate(file.name);
  } else {
    // Use generic comprehensive template for all other PDFs
    template = generateGenericTemplate(file.name);
  }
